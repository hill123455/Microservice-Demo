...
public class UserDetailsServiceImpl implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		final List<AppUser> users = Arrays.asList(new AppUser(1, "tubean", encoder.encode("12345"), "USER"),
		...
		for (AppUser appUser : users) {
			if (appUser.getUsername().equals(username)) {
				...
				// And used by auth manager to verify and check user authentication. return new
				// User(appUser.getUsername(), appUser.getPassword(), grantedAuthorities);
			}
		}
